...
def setup_ai_analysis(session) -> None:
    ...
    Loads .env, validates JJI_SERVER_URL, sets defaults for AI provider/model.
    Disables analysis if JJI_SERVER_URL is missing or dry-run mode.
    """
...
def enrich_junit_xml(session) -> None:
    """Read JUnit XML, POST to JJI server /analyze-failures, write enriched XML back."""
    ...
    if not xml_path.exists():
        logger.warning("xunit file not found under %s. Skipping AI analysis enrichment", xml_path_raw)
        return
    ...
    try:
        response = requests.post(
            ...
            json={"raw_xml": raw_xml, "ai_provider": ai_provider, "ai_model": ai_model},
