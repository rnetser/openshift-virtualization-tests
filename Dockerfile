...
# Make test_analyzer scripts executable and add symlinks to PATH
RUN find ${TEST_DIR}/scripts/test_analyzer/ \( -name "*.py" -o -name "*.sh" \) -exec chmod +x {} \; && \
    ln -s ${TEST_DIR}/scripts/test_analyzer/pytest_marker_analyzer.py /usr/local/bin/pytest-marker-analyzer && \
    ln -s ${TEST_DIR}/scripts/test_analyzer/compare_coderabbit_decisions.py /usr/local/bin/compare-coderabbit-decisions
