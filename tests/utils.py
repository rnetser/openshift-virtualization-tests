...
def get_vm_cpu_list(vm, admin_client):
    vcpuinfo = vm.get_virt_launcher_pod(admin_client=admin_client).execute(
...
def get_numa_node_cpu_dict(vm, admin_client):
    ...
    """
    out = vm.get_virt_launcher_pod(admin_client=admin_client).execute(command=shlex.split("virsh capabilities"))
...
def get_sriov_pci_address(vm, admin_client):
    ...
    """
    sriov_pci_addresses = []
    hostdev_devices = vm.get_xml_dict(admin_client=admin_client)["domain"]["devices"]["hostdev"]
...
def validate_dedicated_emulatorthread(vm, admin_client):
    ...
    cputune = vm.get_xml_dict(admin_client=admin_client)["domain"]["cputune"]
