...
def key_is_in_cputune(vm, key, admin_client):
    ...
    return key in vm.get_xml_dict(admin_client=admin_client)["domain"][CPUTUNE]
...
def check_vcpupin_count(vm, admin_client):
    ...
    vcpupin_count = len(vm.get_xml_dict(admin_client=admin_client)["domain"][CPUTUNE]["vcpupin"])
...
def cputune_is_in_dumpxml(high_performance_vm, admin_client):
    ...
    if CPUTUNE not in high_performance_vm.get_xml_dict(admin_client=admin_client)["domain"]:
