...
def enable_bitlocker(vm):
    ...
    run_ssh_command(vm=vm, commands=shlex.split('powershell -c "initialize-tpm"'))
    run_ssh_command(vm=vm, commands=shlex.split("manage-bde -on c: -s"))
