...
def execute_virsh_qemu_agent_command(vm, command, admin_client):
    ...
    output = vm.get_virt_launcher_pod(admin_client=admin_client).execute(
...
def check_vm_xml_clock(vm, admin_client):
    ...
    clock_timer_list = vm.get_xml_dict(admin_client=admin_client)["domain"]["clock"]["timer"]
...
def check_vm_xml_tablet_device(vm, admin_client):
    ...
    tablet_dict_from_xml = [
        i for i in vm.get_xml_dict(admin_client=admin_client)["domain"]["devices"]["input"] if i["@type"] == "tablet"
    ][0]
