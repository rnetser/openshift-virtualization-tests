...
def find_coderabbit_decision(comments: list[dict]) -> CodeRabbitDecision:
    ...
    for comment in comments:
        ...
        # - Run smoke tests: `False`
        # - **Run smoke tests:** True
        match = SMOKE_TEST_PATTERN.search(body)
        if match:
            ...
            logger.debug("Test Execution Plan found but no smoke test decision matched")
...
def compare_pr(
    ...
    coderabbit = find_coderabbit_decision(comments)
    ...
    # Run local analyzer
    analyzer = run_analyzer(repo, pr_number, token)
...
def generate_markdown_report(results: list[ComparisonResult], repo: str, *, detailed: bool = False) -> str:
    ...
    if mismatches:
        ...
        if detailed:
            ...
            for r in mismatches:
                lines.extend(generate_detailed_mismatch_analysis(r))
                lines.append("---")
                lines.append("")
...
def main() -> int:
    ...
    results = []
    for pr in prs:
        result = compare_pr(args.repo, pr, token)
    ...
    if args.output == "json":
        ...
        output = generate_markdown_report(results, args.repo, detailed=args.detailed)
